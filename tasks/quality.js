module.exports = function(grunt) {
  ...
  var defaultPatterns = [
    '<%= config.srcPaths.drupal %>/{modules,profiles,libraries,www}/**/*.{php,module,inc,install,profile}',
    '!<%= config.srcPaths.drupal %>/{modules,profiles,libraries,www}/**/*.{box,pages_default,panels_default,views_default,panelizer,strongarm}.inc',
    '!<%= config.srcPaths.drupal %>/{modules,profiles,libraries,www}/**/*.features.*inc',
    '!<%= config.srcPaths.drupal %>/{modules,profiles,libraries,www}/**/vendor/**'
  ];
  ...
  phpcsPatterns.push('!<%= config.srcPaths.drupal %>/{modules,profiles,libraries,static}/**/*.tpl.php', '!**/node_modules/**');
  ...
  var phpcbfConfig = grunt.config.get('config.phpcbf');
  grunt.config('phpcbf', {
    options: {
      bin: phpcbfConfig.path || 'vendor/bin/phpcbf',
      standard: phpStandard
    },
    files: {
      src: phpcs,
  ...
  grunt.registerTask('dcr', 'Run DCR.', function() {
    grunt.loadNpmTasks('grunt-shell');
    grunt.config('shell', {
      dcr: {
  			command: 'vendor/bin/dcr --explain',
  		}
    });
    grunt.task.run('shell:dcr');
  });

  grunt.registerTask('dcr-fix', 'Run DCR --fix.', function() {
    grunt.loadNpmTasks('grunt-shell');
    grunt.config('shell', {
      dcrfix: {
  			command: 'vendor/bin/dcr fix',
  		}
    });
    grunt.task.run('shell:dcrfix');
  });
