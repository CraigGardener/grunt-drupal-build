...
module.exports = function(grunt) {
  ...
  module.loadDrushStatus = function(callback) {
    ...
    grunt.util.spawn({cmd: module.drushPath(), args: ['-r', grunt.config('config.buildPaths.develop'), 'status', '--format=json']}, function(error, result, code) {
      var json = null;
      if (error) {
  ...
  module.profilePath = function() {
    return path.join(grunt.config('config.buildPaths.develop'), 'profiles');
  };

  module.sitePath = function() {
    return path.join(grunt.config('config.buildPaths.develop'), 'sites');
  };

  module.drupalPath = function(type) {
    switch (module.majorVersion()) {
      case 8:
        return path.join(grunt.config('config.buildPaths.develop'), type);
      case 7:
      default:
        return path.join(grunt.config('config.buildPaths.develop'), 'sites/all', type);
    }
  };
  ...
  module.loadDatabaseConnection = function(callback) {
    ...
    grunt.util.spawn({cmd: module.drushPath(), args: ['-r', grunt.config('config.buildPaths.develop'), 'sql-connect']}, function(error, result) {
      if (error) {
